...
## v2 — Telegram 가독성/데이터 신뢰도 개선

### 1) 알트불장 화재경보기
- Binance 유동성 알트 20개를 목표로 하되, 조회 실패 종목은 다음 유동성 종목으로 자동 보충합니다.
- 그래도 20개를 못 채우면 `성공 표본 / 20` 비율만큼 Binance 보조지표 가중치를 자동 축소합니다.
- 따라서 17/20 성공이면 예전처럼 `데이터 범위 100%`라고 표시하지 않고 실제 신뢰도를 반영합니다.
- Telegram 요약에 `데이터 신뢰도`, `Binance x/20 성공`, `불꽃 후보 TOP 5`가 표시됩니다.

### 2) 뉴스 링크
- 코인/한국/미국 TOP 5 뉴스마다 제목 바로 아래에 원문 URL이 출력됩니다.
- Telegram에서 URL을 눌러 바로 열 수 있습니다.

### 3) Telegram 출력 정리
- tqdm 진행률, `15/150`, `it/s`, 종목별 검사 진행 로그, pandas Styler 메모리 주소 등은 요약에서 제거합니다.
- 성공한 작업에서는 `notebook_output.txt`, `runner.log` 원문을 Telegram으로 보내지 않습니다.
- CSV/JSON/XLSX/PNG/PDF 같은 실제 결과물만 첨부합니다.
- 실패/시간초과일 때만 디버깅용 로그를 첨부합니다.
- 주식/코인 전략 모음은 후보/판정/레짐/오류 등 핵심 결과만 압축해서 표시합니다.
